"""
Quick CUM v3 benchmark: Soft NS + Cautious Masking vs Muon baseline.
2000 steps on TinyShakespeare micro-transformer.
"""
...
def make_model_and_opts(dataset, opt_config):
    """Create fresh model + optimizers for a given config."""
    ...
    if name == "Muon":
        ...
        main_opt = CUMv3(
            ...
            cautious=opt_config.get("cautious", False),
...
def main():
    ...
    print("=" * 60)
    print("CUM v3 Benchmark: Soft NS + Cautious vs Muon")
    print("=" * 60)
    ...
    configs = [
        {"name": "Muon"},
        {"name": "CUMv3 soft=0.1", "ns_blend": 0.1},
        {"name": "CUMv3 soft=0.2", "ns_blend": 0.2},
        {"name": "CUMv3 caut+soft=0.1", "cautious": True, "ns_blend": 0.1},
    ]
    ...
    for name, val_loss, elapsed in results:
        print(f"  {name:25s}: val_loss={val_loss:.4f}  time={elapsed:.1f}s")

    muon_loss = results[0][1]
    best_cum = min(results[1:], key=lambda x: x[1])
    delta = muon_loss - best_cum[1]
    if delta > 0:
        print(f"\nBEST CUM: {best_cum[0]} beats Muon by {delta:.4f}")
    else:
        print(f"\nMuon still wins by {-delta:.4f}. Best CUM: {best_cum[0]}")
