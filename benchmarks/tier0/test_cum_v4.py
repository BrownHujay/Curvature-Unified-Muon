"""
CUM v4 benchmark: Stacked innovations vs Muon baseline.
Tests gradient centralization + soft NS + coherence scaling.
"""
...
def make_model_and_opts(dataset, cfg):
    ...
    if name == "Muon":
        ...
        main_opt = CUMv3(hidden_2d, lr=0.02, ns_steps=5,
                         ns_blend=cfg.get("ns_blend", 0.0))
    else:
        main_opt = CUMv4(
            hidden_2d, lr=0.02, ns_steps=5,
            ns_blend=cfg.get("ns_blend", 0.1),
            coherence_alpha=cfg.get("coherence_alpha", 0.3),
            centralize=cfg.get("centralize", True),
        )
...
def main():
    ...
    print("=" * 60)
    print("CUM v4: Stacked Innovations vs Muon")
    print("=" * 60)
    ...
    configs = [
        ...
        {"name": "v4 full",
         "ns_blend": 0.1, "coherence_alpha": 0.3, "centralize": True},
        {"name": "v4 no-center",
         "ns_blend": 0.1, "coherence_alpha": 0.3, "centralize": False},
        {"name": "v4 center-only",
         "ns_blend": 0.0, "coherence_alpha": 0.0, "centralize": True},
    ]
    ...
    muon_time = results[0][2]
    for name, val_loss, elapsed in results:
        delta = val_loss - muon_loss
        print(f"  {name:25s}: val={val_loss:.4f} ({delta:+.4f})  time={elapsed:.0f}s")

    best_cum = min(results[1:], key=lambda x: x[1])
    improvement = muon_loss - best_cum[1]
    print(f"\nBEST: {best_cum[0]} — {'beats' if improvement > 0 else 'loses to'} Muon by {abs(improvement):.4f}")
    print("=" * 60)
